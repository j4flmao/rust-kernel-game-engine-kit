//! X11 presentation engine: the minimal real Vulkan pipeline that owns a
//! device + swapchain and presents cleared frames.
//!
//! Bring-up is fully native: loader -> instance (KHR surface extensions) ->
//! physical-device selection (also present-capable) -> logical device ->
//! X11 surface -> swapchain -> per-frame semaphore/fence/command state.
//! `present_frame` runs the acquire/record/clear/submit/present chain.
//!
//! # Safety
//! `PresentHandles` wraps raw handles owned by the caller (the window
//! subsystem). They must outlive the engine.
#![allow(unsafe_code)] // raw Vulkan + X11 handle plumbing

use core::ffi::c_void;

use super::surface::X11Surface;
...
///   id (cast to a pointer, interpreted as `u64`).
...
impl PresentHandles {
    pub const fn x11(display: *mut c_void, window: u64) -> Self {
        Self {
            device: display as *const c_void,
            window: window as usize as *const c_void,
        }
    }
}
...
/// Owns the native Vulkan presenting pipeline for one window.
///
/// Field order doubles as teardown order: swapchain, surface, logical
/// device, instance (children before parents).
...
impl PresentEngine {
    ...
    pub fn build(
        ...
        let instance = loader.create_instance(&[VK_KHR_SURFACE, VK_KHR_XLIB_SURFACE])?;
        ...
        let surface = unsafe {
            super::surface::create_with_handles(
                ...
                instance.raw(),
                handles.device,
                window_id(handles.window),
            )?
        };
        let devices = unsafe { loader.devices_and_handles(instance.raw()) }?;
        let (physical, info, family) = pick_present_device(&loader, &devices, surface.raw())?;
        ...
        let graphics_queues = info
            .queue_families
            .iter()
            .find(|entry| entry.index == family)
            .map(|entry| entry.queue_count)
            .unwrap_or(1);
        ...
        // SAFETY: handles belong to this live loader/device/surface.
    ...
    /// Applies bounded dirty ranges to the frame-owned upload mirror. The
    /// native submission path can then copy only the valid prefix without
    /// borrowing the simulation/UI tree.
    ...
    pub fn recreate(&mut self, size: PresentSize) -> Result<(), PresentError> {
        ...
        self.extent = extent;
        // Drop framebuffers and image views before the old swapchain images.
    ...
    #[allow(dead_code)]
    const fn graphics_queues(&self) -> u32 {
        self.graphics_queues
    }
}

fn window_id(carrier: *const c_void) -> u64 {
    carrier as usize as u64
}
...
mod tests {
    ...
    #[test]
    fn composite_alpha_falls_back_through_supported_bits() {
        assert_eq!(OPAQUE_COMPOSITE_ALPHA, 0x0000_0001);
        assert_eq!(1u32 << 3u32, 8);
    }

    #[test]
    fn window_id_round_trips_on_64_bit() {
        let id = 0xDEAD_BEEFu64;
        let handles = PresentHandles::x11(core::ptr::null_mut(), id);
        assert_eq!(window_id(handles.window), id);
    }
}
