//! Native Vulkan KHR_swapchain facade for the Windows/Win32 presentation path.
//!
//! All swapchain commands are device-level extension commands resolved
//! through `vkGetDeviceProcAddr` with the Windows calling convention.
#![allow(unsafe_code)] // audited FFI boundary; signatures are Vulkan ABI exact

use core::ffi::c_void;

use super::dl::DlError;
use super::vulkan::*;
...
type VkGetPhysicalDeviceSurfaceCapabilitiesKHR =
    unsafe extern "system" fn(VkPhysicalDevice, VkSurfaceKHR, *mut SurfaceCapabilities) -> VkResult;
type VkGetPhysicalDeviceSurfaceFormatsKHR = unsafe extern "system" fn(
    VkPhysicalDevice,
    VkSurfaceKHR,
    *mut u32,
    *mut SurfaceFormat,
...
type VkCreateSwapchainKHR = unsafe extern "system" fn(
    VkDevice,
    *const SwapchainCreateInfoKhr,
    *const c_void,
    *mut VkSwapchainKHR,
) -> VkResult;
type VkDestroySwapchainKHR = unsafe extern "system" fn(VkDevice, VkSwapchainKHR, *const c_void);
type VkGetSwapchainImagesKHR =
    unsafe extern "system" fn(VkDevice, VkSwapchainKHR, *mut u32, *mut VkImage) -> VkResult;
type VkAcquireNextImageKHR = unsafe extern "system" fn(
    VkDevice,
    VkSwapchainKHR,
    u64,
    VkSemaphore,
...
type VkQueuePresentKHR = unsafe extern "system" fn(VkQueue, *const PresentInfoKhr) -> VkResult;
...
type VkCmdClearColorImage = unsafe extern "system" fn(
    VkCommandBuffer,
    VkImage,
    u32,
...
pub unsafe fn surface_supported(
    ...
    // SAFETY: all handles live and output pointer valid.
...
impl Swapchain {
    ...
    pub unsafe fn recreate(
        ...
        // Since `_old` is nulled out in the replacement info, the driver
        // destroys the previous swapchain; we adopt the new one verbatim.
        _ = _old;
        Ok(())
    }
}
...
mod tests {
    ...
    fn caps_with_max_clamp_respects_driver_limit() {
        ...
        surface_caps.min_image_count = 2;
        surface_caps.max_image_count = 3;
        assert_eq!(default_image_count(&surface_caps), 3);
    }
